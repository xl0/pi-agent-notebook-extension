...
test("runNotebookSummary reports cleared outputs", async () => {
	...
	try {
		...
		expect(summary.content[0]?.text).toContain('<output cell_id="95cca932" index="0" type="execute_result" mime="text/plain" />')
	} finally {
...
test("runNotebookSummary works on fixture path", async () => {
	...
	expect(result.content[0]?.text).toContain('<output cell_id="4ea6855e" index="0" type="stream" name="stdout" />')
})
